...
def _main(instaloader: Instaloader, targetlist: List[str],
    ...
    try:
        ...
        for target in targetlist:
            ...
            with instaloader.context.error_catcher(target):
                if re.match(r"^@[A-Za-z0-9._]+$", target):
                    ...
                    if list_only:
                        from .structures import Hashtag
                        hashtag = Hashtag.from_name(instaloader.context, target[1:])
                        instaloader.list_posts_to_file(hashtag.get_posts_resumable(), target, 
                    ...
                    if list_only:
                        instaloader.list_posts_to_file(instaloader.get_feed_posts(), ":feed", 
                                                      max_count=max_count, post_filter=post_filter)
                    else:
                    ...
                    if list_only:
                        instaloader.list_saved_posts(max_count=max_count, post_filter=post_filter,
        ...
        if list_only and profiles:
            # List URLs for each profile
            for profile in profiles:
                instaloader.list_posts_to_file(profile.get_posts(), profile.username, 
                                              max_count=max_count, post_filter=post_filter)
        else:
...
def main():
    ...
    g_how.add_argument('--list-only', action='store_true',
                       help='Export URLs of posts to a text file instead of downloading content. '
                            'Works with :saved and other targets. Creates a file named {target}_urls.txt.')
