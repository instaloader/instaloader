...
class Instaloader:
    ...
    @_requires_login
    def list_saved_posts(self, max_count: Optional[int] = None,
                         post_filter: Optional[Callable[[Post], bool]] = None,
                         filename: str = "saved_urls.txt") -> None:
        """List URLs of user's saved posts to a text file.

        :param max_count: Maximum count of posts to list
        :param post_filter: function(post), which returns True if given post should be listed
        :param filename: Name of the text file to save URLs to
        :raises LoginRequiredException: If called without being logged in.
        """
        assert self.context.username is not None  # safe due to @_requires_login; required by typechecker
        node_iterator = Profile.own_profile(self.context).get_saved_posts()
        self.list_posts_to_file(node_iterator, ":saved", max_count, post_filter, filename)
    ...
    def list_posts_to_file(self, posts: Iterator[Post], target: str,
                          max_count: Optional[int] = None,
                          post_filter: Optional[Callable[[Post], bool]] = None,
                          filename: Optional[str] = None) -> None:
        """List URLs of posts to a text file.

        :param posts: Iterator of posts to list
        :param target: Target name for filename generation
        :param max_count: Maximum count of posts to list
        :param post_filter: function(post), which returns True if given post should be listed
        :param filename: Name of the text file to save URLs to. If None, generates from target.
        """
        if filename is None:
            # Generate filename from target, removing special characters
            safe_target = target.replace(":", "").replace("#", "hashtag_").replace("%", "location_").replace("@", "followees_")
            filename = f"{safe_target}_urls.txt"
        
        self.context.log(f"Retrieving {target} post URLs...")
        ...
        for post in posts:
            ...
            if post_filter is not None:
                try:
                    ...
                    self.context.error("Post skipped. Filter evaluation failed: {}".format(err))
                    continue
            
            url = f"https://www.instagram.com/p/{post.shortcode}/"
            urls.append(url)
            count += 1
            self.context.log(f"[{count}] {url}")
        ...
        self.context.log(f"Saved {len(urls)} post URLs to {filename}")